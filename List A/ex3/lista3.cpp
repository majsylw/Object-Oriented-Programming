...
namespace maj {
    ...
    :  nastepny(nullptr), poprzedni(nullptr), wartosc(T())
    {}

    template<typename T>
    Lista<T>::Element::Element(T val)
    : nastepny(nullptr), poprzedni(nullptr), wartosc(val)
    {}
    ...
    template<typename T>
    Lista<T>::Iterator::Iterator(Element* el, Lista<T>* lis)
    : aktualny(el), lista(lis)
    {}
    ...
    template<typename T>
    typename Lista<T>::Iterator& Lista<T>::Iterator::operator--() //predekrementacja
    {
        if(aktualny != nullptr)
        {
    ...
    template<typename T>
    typename Lista<T>::Iterator Lista<T>::Iterator::operator--(int) //postdekrementacja
    {
        Iterator *kopia = this;
        --(*this);
        return *kopia;
    ...
    typename Lista<T>::Iterator Lista<T>::end()
    {
        ...
        if(ostatni == nullptr)
            return Iterator(ostatni,this);

        while( ostatni->nastepny != nullptr) {
            ostatni = ostatni->nastepny;
        }
        return Iterator(ostatni->nastepny,this);
    }
        ...
        if(iter.Aktualny() != nullptr){
            ...
            if( tymczasowy == iter.Aktualny() ) {
                ...
                if(tymczasowy->nastepny == iter.Aktualny()) {
                    ...
                    (iter.Aktualny()->nastepny)->poprzedni = tymczasowy;
                    delete iter.Aktualny();
                }
            }
            --rozmiar;
...
auto test_lista(void) -> void
{
    ...
    maj::Lista<char>::Iterator c = s1.begin();
    c++;
    ++c;
    c--;
    assert(*(c) == 'b');
